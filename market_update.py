...
PROFIT_MIN_IMPS = 20000
EXCLUDE_NAME_KEYWORDS = [
    "Boots","Gloves","Gauntlets","Armor","Fairy",
    "Shadow Gem","Potion of Monkey","Lustrious Gem","Candy","Sword","Blueprint"
]
SHOW_MAX_ROWS = 30
...
def _blocked_by_name(name: str) -> bool:
    nm = (name or "").lower()
    return any(kw.lower() in nm for kw in EXCLUDE_NAME_KEYWORDS)
...
def main():
    ...
    for r in current_sell_rows:
        iid, price, qty, name = r["item_id"], r["price"], r["quantity"], r["item_name"]
        if _blocked_by_name(name): continue
        if iid not in hist_avgs:   continue

        avg, obs = hist_avgs[iid]
        if obs < MIN_OBS or avg <= 0: continue
        ...
        if discount_pct >= THRESHOLD_DISCOUNT_PCT:
            discount_hits.append({
                ...
                "price": int(price), "avg": round(float(avg), 2),
                "obs": int(obs), "qty": int(qty),
                "seller": r["seller"], "stall": r["stall"],
            })
        ...
        if profit_total >= PROFIT_MIN_IMPS:
            profit_hits.append({
                ...
                "seller": r["seller"], "stall": r["stall"],
                "price": int(price), "avg": round(float(avg), 2),
                "profit_unit": round(profit_unit, 2), "qty": int(qty),
                "profit_total": round(profit_total, 2),
                "discount_pct": round(discount_pct, 2), "obs": int(obs),
            })

    # Descuentos (siempre escribe CSV)
    if discount_hits:
        ...
        df_disc = pd.DataFrame(columns=["discount_pct","item_id","item_name","price","avg","obs","qty","seller","stall"])
        print("\nNo SELL items with ≥ 50% discount.")
    df_disc.to_csv(RESULTS_DISCOUNT_CSV, index=False)

    # Profit (siempre escribe CSV)
    if profit_hits:
        ...
        df_profit = pd.DataFrame(columns=[
            "date","item_id","item_name","seller","stall","price","avg",
            "profit_unit","qty","profit_total","discount_pct","obs"
        ])
